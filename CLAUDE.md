...
- 古诗文专家团（122篇）**已于 2026-06-29 移出本项目**，迁到 `../可心家庭学习站/古诗文/`（由家庭学习站统一托管），本项目不再承载它。
...
古诗文专家团（122篇逐篇多角度亲子共读，七人工作流 + `poetry-expert-panel` skill）原是本项目衍生出来的内容，曾作为子目录挂在这里。**2026-06-29 已物理迁出**，改由「可心家庭学习站」统一托管：

➡️ **`../可心家庭学习站/古诗文/`**（仍是独立 git 仓库，历史完整；家庭学习站负责把它发布到家庭内网）

要做古诗文相关工作，进那个新目录、看它自己的 `CLAUDE.md` 与 `README.md`。本项目这边只保留《如梦令》家庭分享第1期的素材（`sessions/2026-06/260601_55_如梦令/` 的 `01-*` 文件 + `历史文档/` 的 AI 实验视频）——它们是家庭分享产物，不随专家团迁走。
